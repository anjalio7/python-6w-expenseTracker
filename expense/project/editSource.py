...
# field in left side frame
        ...
        res = database.singleSource(self.sourceId)
        if res:
            self.sourceEntry.insert(0, res[0])
            self.incomeEntry.insert(0, res[1])
        else:
    ...
    def sources1(self):
        if self.sourceEntry.get() == '':
            messagebox.showerror('Alert', 'Please enter income source')
        elif self.incomeEntry.get() == '':
            messagebox.showerror('Alert', 'Please enter income')
        else:
            self.data = (
                self.sourceEntry.get(),
                self.incomeEntry.get(),
                self.sourceId[0]
            )
            print(self.data)
            res = database.updateSource(self.data)
            if res:
                self.root.destroy()
