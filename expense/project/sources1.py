...
# add button
    ...
    def sources1(self):
        ...
        for i in range(self.i):
            if globals()[f'self.sourceEntry{i}'].get() == '' or globals()[f"self.incomeEntry{i}"].get() == '':
                messagebox.showerror('Alert', 'Please enter details.')
            else:
                sourceList.append((self.userData[0], globals()[f'self.sourceEntry{i}'].get(), globals()[f"self.incomeEntry{i}"].get()))
        ...
        if self.i == 0:
            ...
            for i in sourceList:
                self.data = i
                print(self.data)
                res = database.addSources(self.data)
                if res:
                    messagebox.showinfo('Success', 'Sources added successfully.')
                    self.root.destroy()
